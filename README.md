...
Home Assistant Setup zur zentralen Überwachung und Steuerung einer PV-Anlage mit **Hoymiles HMS-1600-4T** (Mikrowechselrichter) und **SMA Wechselrichter** — inkl. automatischer Heizstab-Steuerung per Solar-Überschuss, Mosquitto MQTT Broker und Solar-Dashboard.
...
| Hoymiles DTU-Lite | ESP32-basiert |
| SMA Wechselrichter | SMA (mit Webconnect-Schnittstelle) |
| SMA Sunny Home Manager 2.0 | Speedwire UDP Multicast |
| Home Server | Linux Mint 22.3 Notebook |
...
| `sensor.solar_ueberschuss` | PV Gesamt − Grundlast (nur positive Werte) |
| `sensor.grundlast` | Netzbezug aus `sensor.sma_metering_active_power_draw_l1` |

Alle Werte in Watt, basierend auf `sensor.sma_grid_power` des SMA Wechselrichters.
...
**3-Phasen Shelly-Steuerung:**
- 3× Shelly Plus 1PM in der Zuleitung des Heizstabs (L1/L2/L3)
- HA steuert `switch.heizstab_l1/l2/l3` direkt per lokaler API
- Rotation der aktiven Phase bei 1-Phasen-Betrieb für gleichmäßige Belastung

**Automations-Logik** (`config/automations.yaml`):

| Schwelle | Phasen |
|---|---|
| Überschuss > 3500 W | 3 Phasen |
| Überschuss > 2000 W | 2 Phasen |
| Überschuss > 800 W | 1 Phase |
| Batterie < 80 % | Aus |

Läuft alle 2 Minuten zwischen 07:00 und 19:00 Uhr. Die Automation lässt sich per `input_boolean.heizstab_automation_aktiv` im Dashboard deaktivieren.
...
**Energie & Heizstab:**
- Gauges: PV Gesamt / Solar-Überschuss / Grundlast
- Heizstab-Steuerung: Automation-Toggle, Shelly-Schalter L1/L2/L3

## Fernzugriff via Cloudflare Tunnel

Home Assistant ist von außen erreichbar über einen Cloudflare Tunnel — ohne offene Ports im Router, gesichert durch Cloudflare Zero Trust Access.

### Architektur

```
Heimnetz  → http://<server-ip>:8123   → kein Login (trusted_network)
Internet  → https://ha.example.de     → Cloudflare Access → CF Tunnel → HA
```
...
- Service Token sicher aufbewahren — ebenfalls nicht committen
...
Home Assistant ist dann erreichbar unter `http://<server-ip>:8123`
...
- Der **SMA Sunny Home Manager 2.0** kommuniziert ausschließlich über Speedwire UDP Multicast und hat keine Webconnect-Schnittstelle. Die Netzleistung wird stattdessen über den SMA Wechselrichter (`sensor.sma_grid_power`) ausgelesen.
- Die SMA-Entity-IDs (`sensor.sma_*`) sind nach der Installation in HA umzubenennen — standardmäßig enthält HA die Seriennummer im Entity-ID.
